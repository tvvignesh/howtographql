...
In `index.ts`, extend the `typeDefs` string as follows:

```ts{7-9}(path="../hackernews-ts/src/index.ts")
const typeDefs = `
  type Query {
    info: String!
    feed: [Link!]!
...
With that new file in place, you can cleanup `index.ts` a bit.

<Instruction>

First, entirely delete the definition of the `typeDefs` constant - it's not needed any more because the schema
definition now lives in its own file. Then, we build the schema using the typeDefs as loaded from an external file instead of using the constant:

```ts{5-8}(path="../hackernews-ts/src/index.ts)
import fs from 'fs';
...
```ts{7,13-24}(path="../hackernews-ts/src/index.ts")
let links = [{
  id: 'link-0',
