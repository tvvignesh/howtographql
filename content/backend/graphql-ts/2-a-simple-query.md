...
In `index.ts`, update the `typeDefs` constant to look as follows:

```ts{4,7-11}(path="../hackernews-ts/src/index.ts")
const typeDefs = `
  type Query {
    info: String!
    feed: [Link!]!
...
In `index.ts`, add a new list with dummy data as well and update the `resolvers` to look as follows:

```ts{2-6,12,15-19}(path="../hackernews-ts/src/index.ts")
// 1
let links = [{
  id: 'link-0',
...
running, then execute `yarn dev` again) and navigate to `http://localhost:4000` in your browser. If you expand
